...
btnSortByDate.Click += (s, e) => SortItemsByDate();

// IMPORTANT: add BEFORE loading items
this.Controls.Add(btnSortByDate);
btnSortByDate.BringToFront();

//// PART 2 END — SORT BUTTON UI ////
...
Button btnSortByDate = new Button
{
    ...
    FlatStyle = FlatStyle.Flat,
    Font = new Font("Arial", 10, FontStyle.Bold),
    Location = new Point(20, 10) // top-left of gallery area
};

// click event → call sorting function
btnSortByDate.Click += (s, e) => SortItemsByDate();

// attach to the outer panel so it stays on left side
outerPanel.Controls.Add(btnSortByDate);
btnSortByDate.BringToFront();
